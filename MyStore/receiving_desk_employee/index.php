...
try
{
	...
	foreach($res as $row)
		{
		$output.= "<table border='1'><tr><td><a href='update.php?id=".$row[0]."'>".$row[0]."</td><td>".$row[1]."</td><td>".$row[2]."</td></tr></table>";
		$id=$row['number'];
		}
	}
	...
	{
	$descr1=$_POST['descr'];
	$sql1="select * from parts where description like '%".$descr1."%';";
	...
	if($res1)
	{
		...
		foreach($res1 as $row)
		{
			$output.= "<tr><td><a href='update.php?id=".$row[0]."'>".$row[0]."</a></td><td>".$row[1]."</td></tr>";
			//	$id=$row[0];
		}
		$output.="</table>";
	}
