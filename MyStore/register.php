...
{
	$prod_id_array=unserialize($_POST['custom1']);
}
...
foreach($res as $row)
	...
	if($check!=$id)
	{
	  $sql="insert into register values('$title','$fname','$lname','$gen','$id','$pass','$phone','$add','$city','$coun','$dob')";
	$res=$conn2->query($sql);
   if($res)
	   {
		 echo "<script>location.href='ship_details.php?wel=$id'</script>";
	   }
	}
else 
{
